...
public interface TeamMapper {
    ...
    @Select("SELECT id, name, owner, time FROM team WHERE id = #{id}")
    Team getById(@Param("id") long id);

    @Select("SELECT id, name, owner, time FROM team WHERE name = #{name}")
    Team getByName(@Param("name") String name);

    @Insert("INSERT INTO team(name, owner) VALUES (#{name}, #{owner})")
